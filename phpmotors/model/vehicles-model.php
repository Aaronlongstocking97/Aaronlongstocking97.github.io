...
{
    $db = phpmotorsConnect();
    $sql = 'SELECT * FROM inventory WHERE invId INNER JOIN (SELECT invId FROM images WHERE imgPrimary = :imgPrimary WHERE imgPath LIKE "%tn%")';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':imPrimary', $imgPrimary, PDO::PARAM_STR);
    $stmt->execute();
