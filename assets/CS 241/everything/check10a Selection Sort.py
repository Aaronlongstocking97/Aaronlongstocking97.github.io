"""
 Program:
    CS241 Checkpoint 10a, Selection Sort 
 Instructor:
    Borther Macbeth
 Author:
    Aaron Jones
 Summary: 
    This program will take in any number value
    from the user and sort it from smallest to
    the largest number.
...
def sort(numbers):
    ...
    for sort_pos in range(len(numbers) -1, 0, -1):
        ...
        for swap_pos in range(sort_pos + 1):

            if numbers[swap_pos] > numbers[max_pos]:
                max_pos = swap_pos
