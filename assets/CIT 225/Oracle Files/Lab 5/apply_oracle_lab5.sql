...
COLUMN contact_id               FORMAT 9999 HEADING "Contact|ID #"
...
COLUMN contact_id               FORMAT 9999 HEADING "Contact|ID #"
COLUMN address_id               FORMAT 9999 HEADING "Address|ID #"
...
COLUMN address_id               FORMAT 9999 HEADING "Address|ID #"
COLUMN street_address_id        FORMAT 9999 HEADING "Street|Address|ID #"
...
COLUMN address_id               FORMAT 9999 HEADING "Address|ID #"
COLUMN telephone_id             FORMAT 9999 HEADING "Telephone|ID #"
SELECT address_id, telephone_id
FROM address a INNER JOIN telephone t 
USING (address_id);
...
COLUMN contact_id               FORMAT 9999 HEADING "Contact|ID #"
COLUMN system_user_id           FORMAT 9999 HEADING "System|User|ID #"
SELECT contact_id, system_user_id
FROM contact c INNER JOIN  system_user
s
ON c.created_by = system_user_id; 
...
WHERE c.created_by = system_user_id;
...
FROM contact c INNER JOIN  system_user
s
ON c.last_updated_by = system_user_id;
...
SELECT c.contact_id, s.system_user_id
FROM contact c, system_user s
WHERE c.last_updated_by = system_user_id;


-- ----------------------------------------------------------
--  Step #3
--  -------
--  Write INNER JOIN queries that use the ON subclause to 
--  perform a self-join on the SYSTEM_USER table. 
--  The solution requires that you create three copies of 
--  the SYSTEM_USER table by using aliases like 
--  su1, su2, and su3.
--  Please note that joining two tables is like matching 
--  two sets, while joining three table is like joining two 
--  tables into a temporary result set (or pseudo table) and 
--  then joining the third table to the temporary result set
...
FROM system_user su1 INNER JOIN  system_user
su2 
...
--  Step #4
--  -------
--  Display the RENTAL_ID column from the RENTAL table, 
--  the RENTAL_ID and ITEM_ID from the RENTAL_ITEM table, 
--  and ITEM_ID column from the ITEM table. You should make 
--  a join from the RENTAL table to the RENTAL_ITEM table, 
--  and then the ITEM table. Join the tables based on their
--  respective primary and foreign key values.
-- ----------------------------------------------------------


-- Part A

COL r_rental_id   FORMAT 999999  HEADING "Rental|Table|--------|Rental|ID #"
COL ri_rental_id  FORMAT 999999  HEADING "Rental Item|Table|-----------|Rental|ID #"
COL ri_item_id    FORMAT 999999  HEADING "Rental Item|Table|-----------|Item|ID #"
COL i_item_id     FORMAT 999999  HEADING "Item|Table|--------|Item|ID #"
SELECT r.rental_id         AS r_rental_id
...
/* Alter the RENTAL table by dropping the NN_RENTAL_3 constraint. */
ALTER TABLE rental 
...
--  Step #5
--  -------
--  Display the DEPARTMENT_NAME from the DEPARTMENT table and 
--  the average in whole dollars of the SALARY column from 
--  the SALARY table grouped by and ordered by the 
--  DEPARTMENT_NAME column for the last two months. 
--  (HINT: The lookup values and range values with a BETWEEN 
--  operator must be not null values, and when the ending 
--  date is null you should use an 
--  NVL(some_date, SYSDATE + 1) or tomorrow to get the 
...
DECLARE
  ...
  TYPE xname IS TABLE OF VARCHAR2
(20);
  ...
  lv_first XNAME := xname
('Ann','Abbey','Amanda','Archie','Antonio','Arnold'
  ...
  lv_last  XNAME := xname
('Abernathy','Anderson','Baker','Barney'
  ...
  lv_dept  XNAME := xname
('Accounting','Operations','Sales','Factory','Manufacturing');
 
  /* A colleciton of possible salaries. */
  lv_salary  XSALARY := xsalary
( 36000, 42000, 48000, 52000, 64000 );
 
  /* Define a local function. */
  FUNCTION random_foreign_key
RETURN INTEGER
IS
...
BEGIN
  ...
  FROM (SELECT ROUND(dbms_random.VALUE(1,1000)/100/2,0) num
    FROM dual) il;
...
BEGIN
  ...
  (SELECT table_name
  FROM user_tables
  WHERE  table_name = 'MOCK_CALENDAR')
  LOOP
...
DECLARE
  ...
  TYPE smonth IS TABLE OF VARCHAR2
(3);
  TYPE lmonth IS TABLE OF VARCHAR2
(9);
 
  /* Declare month arrays. */
  short_month SMONTH := smonth
('JAN','FEB','MAR','APR','MAY','JUN'
                              ,'JUL','AUG','SEP','OCT','NOV','DEC');
  long_month  LMONTH := lmonth
...
FROM employee e INNER JOIN department d
  ON       e.department_id = d.department_id INNER JOIN salary s
  ON       e.salary_id = s.salary_id
WHERE    NVL(effective_start_date, TRUNC(SYSDATE) + 2) BETWEEN TRUNC(SYSDATE) - 60 AND TRUNC(SYSDATE) + 1
GROUP BY d.department_name
ORDER BY d.department_name;
