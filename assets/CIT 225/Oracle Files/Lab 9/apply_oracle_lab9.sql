...
,
  CONSTRAINT fk_transaciton_1   FOREIGN KEY(transaction_type) REFERENCES common_lookup(common_lookup_id)
,
...
-- Add new rows to the AIRPORT table
INSERT INTO airport
  ( airport_id
  , airport_code
  , airport_city
...
ROLLBACK
TO SAVEPOINT all_or_none;
    RAISE_APPLICATION_ERROR
(
       -20001
      ,'Remove the NOT NULL contraint from the '||lv_column_name||' column in'||CHR
(10)||' the '||lv_table_name||' table.');
  WHEN OTHERS THEN
/* This undoes all DML statements to this point in the procedure. */
ROLLBACK
TO SAVEPOINT all_or_none;
...
('You have an error in your AIRPORT table inserts.');
