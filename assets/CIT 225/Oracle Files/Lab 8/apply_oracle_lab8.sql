...
FROM
  (SELECT i.item_id
     ...
     , CASE
      ...
      (TRUNC(SYSDATE) - i.release_date) > 30 AND af.active_flag = 'N' 
                THEN i.release_date
    ...
    WHERE  su.system_user_name = 'SYSADMIN') AS created_by
     ...
     , (SELECT su.system_user_id
    FROM system_user su
    WHERE  su.system_user_name = 'SYSADMIN') AS last_updated_by
  ...
  FROM item i CROSS JOIN
    ...
    UNION ALL
      ...
      FROM dual
    UNION ALL
      SELECT '5' AS rental_days
      FROM dual) dr INNER JOIN
...
MODIFY
(rental_item_price NUMBER CONSTRAINT nn_rental_item_price_8 NOT NULL);
