...
@
/home/student/Data/cit225/oracle/lab2/cleanup_oracle_lab.sql
...
WHERE    uc.table_name = 'SYSTEM_USER_LAB'
...
FROM user_sequences
WHERE    sequence_name = 'SYSTEM_USER_LAB'||'_S1';
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'COMMON_LOOKUP_LAB'
...
WHERE    ui.table_name = 'COMMON_LOOKUP_LAB'
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'COMMON_LOOKUP_LAB'||'_S1';
...
(
  member_lab_id NUMBER CONSTRAINT nn_member_lab_1 NOT NULL
,
  member_type NUMBER       --CONSTRAINT nn_member_lab_2 NOT NULL
,
  account_number VARCHAR2(10) CONSTRAINT nn_member_lab_3 NOT NULL
,
  credit_card_number VARCHAR2(19) CONSTRAINT nn_member_lab_4 NOT NULL
,
  credit_card_type NUMBER CONSTRAINT nn_member_lab_5 NOT NULL
,
  created_by NUMBER CONSTRAINT nn_member_lab_6 NOT NULL
,
  creation_date DATE CONSTRAINT nn_member_lab_7 NOT NULL
,
  last_updated_by NUMBER CONSTRAINT nn_member_lab_8 NOT NULL
,
  last_update_date DATE CONSTRAINT nn_member_lab_9 NOT NULL
,
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'MEMBER_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
FROM user_indexes ui INNER JOIN user_ind_columns uic
  ON       ui.index_name = uic.index_name
    AND ui.table_name = uic.table_name
WHERE    ui.table_name = 'MEMBER_LAB'
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'MEMBER_LAB'||'_S1';
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'CONTACT_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
FROM user_indexes ui INNER JOIN user_ind_columns uic
  ON       ui.index_name = uic.index_name
    AND ui.table_name = uic.table_name
WHERE    ui.table_name = 'CONTACT_LAB'
  AND NOT  ui.index_name IN (SELECT constraint_name
  FROM user_constraints)
ORDER BY ui.index_name
,        uic.column_position;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'CONTACT_LAB'||'_S1';
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'ADDRESS_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
FROM user_indexes ui INNER JOIN user_ind_columns uic
  ON       ui.index_name = uic.index_name
    AND ui.table_name = uic.table_name
WHERE    ui.table_name = 'ADDRESS_LAB'
  AND NOT  ui.index_name IN (SELECT constraint_name
  FROM user_constraints)
ORDER BY ui.index_name
,        uic.column_position;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'ADDRESS_LAB'||'_S1';
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'STREET_ADDRESS_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'STREET_ADDRESS_LAB'||'_S1';
...
,
  CONSTRAINT fk_telephone_lab_2   FOREIGN KEY(telephone_type) 
  REFERENCES common_lookup_lab(common_lookup_lab_id) 
,
...
, CONSTRAINT fk_telephone_lab_5   FOREIGN KEY
(address_lab_id)  REFERENCES address_lab
(address_lab_id));
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'TELEPHONE_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
FROM user_indexes ui INNER JOIN user_ind_columns uic
  ON       ui.index_name = uic.index_name
    AND ui.table_name = uic.table_name
WHERE    ui.table_name = 'TELEPHONE_LAB'
  AND NOT  ui.index_name IN (SELECT constraint_name
  FROM user_constraints)
ORDER BY ui.index_name
,        uic.column_position;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'TELEPHONE_LAB'||'_S1';
...
FROM user_constraints uc INNER JOIN user_cons_columns ucc
  ON       uc.table_name = ucc.table_name
    AND uc.constraint_name = ucc.constraint_name
WHERE    uc.table_name = 'RENTAL_LAB'
  AND uc.constraint_type IN (UPPER('c'),UPPER('p'))
ORDER BY uc.constraint_type DESC
,        uc.constraint_name;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'RENTAL_LAB'||'_S1';
...
WHERE    uc.constraint_name = ucc1.constraint_name
  ...
  AND ucc1.position = ucc2.position -- Correction for multiple column primary keys.
  AND uc.constraint_type = 'R'
  AND ucc1.table_name = 'RENTAL_ITEM_LAB'
ORDER BY ucc1.table_name
,        uc.constraint_name;
...
SELECT sequence_name
FROM user_sequences
WHERE    sequence_name = 'RENTAL_ITEM_LAB'||'_S1';

--Step 11 
--------------------------------------------------------------------
-- verify the ten tables.
--
--------------------------------------------------------------------
