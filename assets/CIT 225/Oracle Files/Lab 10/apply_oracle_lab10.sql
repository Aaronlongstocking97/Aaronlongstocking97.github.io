...
-- Step 1 SELECT the contact_id from the CONTACT Table
COLUMN   contact_id         FORMAT 9999 HEADING "CONTACT_ID"
...
COLUMN last_updated_by  FORMAT 9999 HEADING "Update|By"
COLUMN last_update_date FORMAT A10  HEADING "Last|Updated"
SELECT DISTINCT
    r.rental_id
, c.contact_id AS customer
...
-- Step 1 Inserts into the RENTAL Table
INSERT INTO rental
...
FROM (SELECT DISTINCT
    ...
    FROM member m INNER JOIN contact c
        ...
        ON       c.contact_id = r.customer_id
            AND tu.check_out_date = r.check_out_date
            AND tu.return_date = r.return_date) r;
...
-- Step 2 Second SELECT Statement
SET NULL
'<Null>'
...
-- Step 2 Inserts into the RENTAL_ITEM Table
